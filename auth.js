...
/*==========================================================*
 * TOYSGURU — AUTH (Firebase)
 * Email/Password + Google + Owner routing
 *==========================================================*/
...
const TOYSGURU_OWNER_EMAIL = "admintoysguru@gmail.com";

function isOwnerAccount(userOrEmail) {
    const email = typeof userOrEmail === "string"
        ? userOrEmail
        : userOrEmail?.email;

    return String(email || "").trim().toLowerCase() === TOYSGURU_OWNER_EMAIL;
}

function getPostLoginDestination(user) {
    return isOwnerAccount(user) ? "admin-dashboard.html" : "marketplace.html";
}

async function ensureOwnerProfile(user) {
    if (!user || !isOwnerAccount(user)) return;
    ...
    if (!snap.exists) {
        await ref.set({
            name: user.displayName || "ToysGuru Owner",
            email: user.email,
...
async function createProfileIfNeeded(
    ...
    if (snap.exists) {
        ...
        /*
         * If this is the dedicated owner account,
         * make sure the existing profile has admin access.
         */

        if (
            isOwnerAccount(user) &&
            snap.data()?.isAdmin !== true
        ) {
            await ref.update({
                isAdmin: true,
                email: user.email,
...
function showAccountCompletion(
    ...
    gateTabs?.classList.add(
        "hidden"
    );

    if (gateTabs) {
        gateTabs.style.display =
            "none";
    }
...
loginForm?.addEventListener(
    ...
    async e => {
        ...
        try {
            ...
            /*
             * THIS is the important owner routing.
             */
...
googleLoginBtn?.addEventListener(
    ...
    async () => {
        ...
        try {
            ...
            /*
             * Owner with Google only.
             *
             * Directly enter the owner dashboard.
             */
            ...
            /*
             * Normal Google-only user.
             *
             * Create their optional
